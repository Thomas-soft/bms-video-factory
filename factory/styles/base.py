...
from factory.core.models import Asset, Channel, Charte, Gabarit, Shot, Shotlist, Style
...
class MoteurBase:
    ...
    @staticmethod
    def gabarit(channel: Channel, run: RunPaths) -> Gabarit:
        """Gabarit de mise en page du run (`manifest.identite.template_id`), défaut sinon."""
        import json

        try:
            identite = json.loads((run.racine / "manifest.json").read_text(encoding="utf-8"))["identite"]
            return channel.charte.layouts.get(identite["template_id"]) or Gabarit()
