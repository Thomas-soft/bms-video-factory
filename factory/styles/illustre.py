...
class IllustreEngine(MoteurBase):
    ...
    def prepare_assets(
        ...
        generateur.charger_heritage(run)
        # Une image héritée du parent (étape 24) n'a pas de prompt à traduire.
        a_traduire = [
            s.asset_request.prompt_or_keywords for s in shotlist.shots
            if s.asset_request.type != "card"
            and not (generateur.heritage and (
                s.asset_request.prompt_or_keywords.strip() in generateur.heritage["par_prompt"]
                or generateur.heritage["par_segment"].get(s.segment_id)))
        ]
        generateur.traductions = assets_images.traduire_intentions(
            a_traduire, channel.lang, racine=self.racine, journal=self.journal
        )

        # Réemploi sémantique (étape 29) : seulement sous le même préfixe de charte.
        try:
            from factory import library

            generateur.semantique = library.IndexSemantique.charger(
                self.conn, "images", tag=library.tag_prefixe(channel.charte.style_prefix))
            generateur.semantique.preparer(
                [generateur.traductions.get(t, t) for t in a_traduire], racine=self.racine)
        except Exception as erreur:  # l'index est un accélérateur, pas une dépendance
            generateur.semantique = None
        ...
        if generateur.heritage:
            (run.racine / "heritage_stats.json").write_text(json.dumps(
                {**generateur.herites, "generees": generees, "plans": len(shotlist.shots)},
                ensure_ascii=False) + "\n", encoding="utf-8")
        ...
        for shot in shotlist.shots:
            if shot.on_screen_text or shot.is_sponsor:
                ...
                if not calque.exists():
                    self._composer_texte(shot, channel, police_titre, police_corps,
                                         self.gabarit(channel, run)).save(
                        calque, "PNG"
                    )
        ...
        self.mesures = {
            ...
            "assets_reused_semantic": generateur.reemplois_semantiques,
    ...
    def _composer_texte(
        ...
        police_corps: video.Police, gabarit=None,
    ) -> Image.Image:
        """Calque RGBA : cartouche, texte à l'écran, souligné d'accent, bandeau de divulgation.

        Le gabarit du run (étape 29) place le cartouche et décide du souligné.
        """
        palette = channel.charte.palette
        ...
        if texte:
            ...
            ancre = gabarit.text_anchor if gabarit else "bottom_center"
            x = MARGE_PX if ancre.endswith("left") else (video.LARGEUR - cartouche.width) // 2
            y = MARGE_PX if ancre.startswith("top") else video.HAUTEUR - MARGE_PX - cartouche.height
            video.coller(calque, cartouche, (x, y))
            video.coller(calque, bloc, (x + MARGE_PX // 2, y + MARGE_PX // 4))
            if gabarit is None or gabarit.accent_bar:
        ...
        # Divulgation en haut à gauche : sous un gabarit « top_left », elle passe à droite.
        if shot.is_sponsor:
            bandeau = self._bandeau_divulgation(channel, police_corps)
            gauche = not (texte and gabarit is not None and gabarit.text_anchor == "top_left")
            video.coller(calque, bandeau, (MARGE_PX if gauche
                                           else video.LARGEUR - MARGE_PX - bandeau.width, MARGE_PX))
        return calque
    ...
    def _rendre_ken_burns(
        ...
        L'image d'entrée doit être bouclée (`-loop 1 -framerate -t`). Sans cela elle ne fournit
        qu'**une seule image**, `zoompan` à `d=1` en rend une, et le `-frames:v` de
        `arguments_encodage` n'a rien à répéter : le clip sort à 0,033 s. Avec un calque de texte
        c'est pire que visible — l'`overlay` répète l'image de fond jusqu'au bout et le clip a la
        bonne durée, **figé**, sans zoom. Mesuré le 16/09/2026 : 99 clips à 0,033 s sur 127.
        """
