...
FICHIERS_POLICES: dict[str, str] = {
    "inter": "Inter[opsz,wght].ttf",
    # Charte « histoire documentaire » (étape 17) : une serif de titre à fort contraste et une
    # serif de texte lisible en petit corps. Les deux sont OFL-1.1, variables, tirées de
    # `github.com/google/fonts` — voir `outils/LICENCES.md` § 4.
    "playfairdisplay": "PlayfairDisplay[wght].ttf",
    "sourceserif4": "SourceSerif4[opsz,wght].ttf",
    # Charte « whiteboard » (étape 30.2) : la seule manuscrite du dépôt. OFL-1.1, variable,
    # tirée de `github.com/google/fonts` — voir `outils/LICENCES.md` § 4.
...
def taille_png(chemin: Path) -> tuple[int, int]:
    """Largeur et hauteur d'une image, sans la charger en mémoire."""
...
def verify_clip(
    ...
    except FfmpegError as erreur:
        # Un clip tronqué par une interruption existe mais n'a pas d'atome `moov` : c'est
        # un écart au contrat, pas une panne. Le rendre tel permet au réemploi de le refaire.
        return [f"{chemin.name} : illisible — {erreur}"]
