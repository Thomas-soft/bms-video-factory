...
from factory.core.paths import RunPaths, racine_projet, assets_exclus
...
def _reutilisable(
    ...
    """
    exclus = assets_exclus(video_id)
    if asset_id in exclus:
        # Remède « assets_differents » de l'étape 22.2 : cet asset a déjà servi à CE run et
        # le banc a refusé la vidéo pour variété insuffisante. Le reprendre annulerait le
        # remède.
        return False, "exclu par la régénération (variété insuffisante)"
    if conn.execute(
...
class GenerateurImages:
    ...
    #: Construction du prompt, à défaut `construire_prompt` (le style vient de la charte). Le
    #: moteur « whiteboard » y pose le sien : la charte des chaînes livrées décrit une
    #: illustration en aplats de couleur, c'est-à-dire l'inverse d'un dessin au trait.
    constructeur_prompt: Callable[[Channel, str, bool], str] | None = None
    #: Déclinaison (étape 24) : `heritage.json` du run enfant — images du parent par prompt
    #: d'asset, puis par segment. Vide pour un run ordinaire.
    heritage: dict[str, Any] = field(default_factory=dict)
    #: Comptes par origine : `prompt` (même demande), `segment` (même segment), rien sinon.
    herites: dict[str, int] = field(default_factory=lambda: {"prompt": 0, "segment": 0})
    _dernier_herite: str | None = None
    #: Réemploi sémantique (étape 29) : index de la bibliothèque sous le préfixe de la charte,
    #: intentions du run déjà encodées. `None` = désactivé (whiteboard, tests).
    semantique: Any = None
    #: `asset_id` servis dans ce passage : un réemploi sémantique n'en ressert jamais un.
    servis: set[str] = field(default_factory=set)
    reemplois_semantiques: int = 0

    def charger_heritage(self, run: RunPaths) -> None:
        """Lit `heritage.json` s'il existe (run enfant d'une déclinaison)."""
        fichier = run.racine / "heritage.json"
        if fichier.exists():
            self.heritage = json.loads(fichier.read_text(encoding="utf-8"))

    def image_heritee(self, shot: Shot) -> tuple[Path, Asset, str] | None:
        """L'image du parent pour ce plan : même demande d'asset, sinon même segment.

        Aucune régénération si l'asset existe (étape 24) : un plan de plus dans un segment
        n'est pas un plan nouveau, il reprend une image du même segment, en rotation. Jamais
        deux fois de suite la même image : deux plans identiques ne font pas une coupe, et le
        banc la compte comme manquante (mesuré sur `c7km` : 39 paires consécutives).
        """
        if not self.heritage:
            return None
        parent = self.racine / "workspace" / "runs" / self.heritage["parent_id"]
        demande = shot.asset_request.prompt_or_keywords.strip()
        par_segment: dict[str, list[str]] = self.heritage.get("par_segment", {})
        rang = int(shot.id.split("_")[-1])
        candidats: list[tuple[str, str]] = []
        exact = self.heritage.get("par_prompt", {}).get(demande)
        if exact is not None:
            candidats.append((exact, "prompt"))
        siens = par_segment.get(shot.segment_id) or []
        candidats += [(siens[(rang + i) % len(siens)], "segment") for i in range(len(siens))]
        if not candidats:
            return None
        # Dernier recours contre la répétition : les images des segments voisins.
        ordre = list(par_segment)
        if shot.segment_id in ordre:
            i = ordre.index(shot.segment_id)
            for voisin in ordre[i + 1:i + 2] + ordre[max(0, i - 1):i]:
                candidats += [(sid, "segment") for sid in par_segment[voisin]]
        premier: tuple[Path, Asset, str] | None = None
        for source_shot, origine in candidats:
            image = parent / "assets" / source_shot / "image.png"
            licence = parent / "assets" / source_shot / "licence.json"
            if not image.exists() or not licence.exists():
                continue
            asset = Asset.model_validate_json(licence.read_text(encoding="utf-8"))
            if asset.has_text:
                continue  # une image lettrée ne franchit pas la frontière de langue
            premier = premier or (image, asset, origine)
            if asset.asset_id != self._dernier_herite:
                premier = (image, asset, origine)
                break
        if premier is None:
            return None
        self._dernier_herite = premier[1].asset_id
        self.herites[premier[2]] += 1
        return premier
    ...
    def image_pour(
        ...
        heritee = self.image_heritee(shot)
        if heritee is not None:
            image, asset, origine = heritee
            cle = f"heritage-{asset.asset_id}"
            self.tracer(f"{shot.id} : hérité de {self.heritage['parent_id']} ({origine})")
            _inscrire(self.conn, asset, image, cle, channel, video_id, self.racine)
            return ResultatImage(image, asset, cle, genere=False, secondes=0.0, essais=0)
        demande = shot.asset_request.prompt_or_keywords.strip()
        ...
        prompt = (self.constructeur_prompt or construire_prompt)(channel, sujet, rupture)
        ...
        if fichier.exists() and fichier.stat().st_size >= TAILLE_MINIMALE_OCTETS and licence.exists():
            ...
            if libre:
                ...
                self.servis.add(asset.asset_id)
        ...
        repris = self._reemploi_semantique(shot, channel, video_id, sujet)
    ...
    def _reemploi_semantique(
        self, shot: Shot, channel: Channel, video_id: str, sujet: str
    ) -> ResultatImage | None:
        """Une image de la bibliothèque dont la description est une paraphrase de l'intention."""
        from factory import library

        trouve = library.reemploi_semantique(
            self.conn, self.semantique, sujet, seuil=channel.library.semantic_threshold,
            libre=lambda a: _reutilisable(self.conn, a, channel, video_id),
            deja_dans_run=self.servis, video_id=video_id, shot_id=shot.id, kind="images",
        )
        if trouve is None:
            return None
        asset_id, relatif, score = trouve
        fichier = self.racine / relatif
        licence = fichier.with_suffix(".json")
        if not fichier.exists() or not licence.exists():
            return None
        asset = Asset.model_validate_json(licence.read_text(encoding="utf-8"))
        cle = fichier.stem
        _inscrire(self.conn, asset, fichier, cle, channel, video_id, self.racine)
        self.servis.add(asset_id)
        self.reemplois_semantiques += 1
        self.tracer(f"{shot.id} : réemploi sémantique {cle} (similarité {score:.3f}) "
                    f"pour « {sujet[:80]} »")
        return ResultatImage(fichier, asset, cle, genere=False, secondes=0.0, essais=0)
