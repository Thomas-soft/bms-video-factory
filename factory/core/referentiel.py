...
def patrons_titres(nom_niche: str, racine: Path | None = None) -> list[dict[str, Any]]:
    """Patrons de titre mesurés de la niche, du plus fréquent au moins fréquent."""
    patrons = list(niche(nom_niche, racine)["titres"].get("patrons") or [])
    return sorted(patrons, key=lambda p: -float(p.get("part") or 0.0))


def cible_titres(nom_niche: str, racine: Path | None = None) -> dict[str, Any]:
    """Bloc `titres` de la niche : longueurs observées et plafonds."""
    return dict(niche(nom_niche, racine)["titres"])


def cible_miniatures(nom_niche: str, racine: Path | None = None) -> dict[str, Any]:
    """Bloc `miniatures` de la niche : mots de texte, contraste visé, motifs gagnants."""
    return dict(niche(nom_niche, racine)["miniatures"])
