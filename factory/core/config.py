...
DOSSIERS: dict[str, tuple[type[ModeleRacine], str]] = {
    ...
    "channels": (Channel, "id"),
    "products": (Product, "id"),
    "voices": (VoiceProfile, "id"),
}

#: Dossiers dont l'absence n'est pas une erreur : sans profils, la voix vient de la langue.
DOSSIERS_FACULTATIFS = {"voices"}
...
FICHIERS: dict[str, type[ModeleRacine]] = {
    ...
    "orchestrator.yaml": OrchestratorConfig,
}
...
def charger_fichier(
    ...
    # Étape 29 : la charte d'une chaîne vit dans `config/chartes/<id>.yaml` quand il existe.
    if modele is Channel and fichier.parent.name == "channels":
        externe = fichier.parent.parent / "chartes" / fichier.name
        if not externe.is_file():  # candidat validé hors de config/ (tableau de bord)
            externe = dossier_config(None) / "chartes" / fichier.name
        if externe.is_file():
            if "charte" in brut:
                return None, [Probleme(nom, f"charte déclarée deux fois : ici et chartes/{nom}")]
            try:
                charte = _lire_yaml(externe)
            except (yaml.YAMLError, ValueError, OSError) as erreur:
                return None, [Probleme(f"chartes/{nom}", str(erreur).splitlines()[0])]
            if charte.pop("channel", fichier.stem) != fichier.stem:
                return None, [Probleme(f"chartes/{nom}", "channel ne correspond pas au fichier")]
            charte.pop("schema_version", None)
            charte["version"] = charte.pop("charte_version", charte.get("version"))
            brut["charte"] = charte
    try:
...
class ConfigSet:
    ...
    def profil_voix(self, channel: Channel) -> VoiceProfile | None:
        """Profil `config/voices/<voice_id>.yaml` de la chaîne, s'il existe."""
        return self.voices.get(channel.voice_id)
...
def charger(racine: Path | None = None, strict: bool = True) -> ConfigSet:
    ...
    for nom_dossier, (modele, cle) in DOSSIERS.items():
        ...
        if not chemin.is_dir():
            if nom_dossier not in DOSSIERS_FACULTATIFS:
...
def croiser(cfg: ConfigSet) -> list[Probleme]:
    ...
    locuteurs: dict[tuple[str, str, str], str] = {}
    licences = cfg.racine.parent / "outils" / "LICENCES.md"
    texte_licences = licences.read_text(encoding="utf-8") if licences.exists() else ""
    for profil in cfg.voices.values():
        if profil.reference_wav:
            nom = Path(profil.reference_wav).name
            if not (cfg.racine.parent / profil.reference_wav).exists():
                problemes.append(Probleme(f"voices/{profil.id}.yaml",
                                          f"reference_wav introuvable : {profil.reference_wav}"))
            if nom not in texte_licences:
                problemes.append(Probleme(f"voices/{profil.id}.yaml",
                                          f"référence {nom} absente de outils/LICENCES.md"))
    for channel in cfg.channels.values():
        ...
        for produit_id in channel.products:
            ...
            # Étape 27 : la surcharge du produit s'AJOUTE à la ligne du réseau (links.mentions),
            # elle n'en dispense plus.
            if not ligne.strip():
                problemes.append(
                    Probleme(
                        fichier,
        ...
        profil = cfg.voices.get(channel.voice_id)
        if cfg.voices and profil is None:
            problemes.append(Probleme(
                fichier, f"voix « {channel.voice_id} » sans profil config/voices/{channel.voice_id}.yaml"))
        elif profil is not None:
            if profil.lang != channel.lang:
                problemes.append(Probleme(
                    fichier, f"profil de voix {profil.id} en {profil.lang}, chaîne en {channel.lang}"))
            # deux identifiants différents peuvent désigner le même locuteur du même moteur
            cle_locuteur = (channel.lang, profil.engine, profil.speaker)
            autre = locuteurs.get(cle_locuteur)
            if autre and autre != channel.id:
                problemes.append(Probleme(
                    fichier, f"locuteur {profil.engine}/{profil.speaker} déjà employé par {autre} "
                    "dans la même langue (CONFORMITE § 5)"))
            locuteurs.setdefault(cle_locuteur, channel.id)
