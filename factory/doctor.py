...
def charge_env() -> None:
    ...
    defauts = {
        ...
        # Playwright lit cette variable au lancement du navigateur : sans elle, il retélécharge
        # 0,21 Go dans ~/Library/Caches (panne rencontrée à l'étape 13.2).
        "PLAYWRIGHT_BROWSERS_PATH": "./models/playwright",
    }
...
def modeles_retenus() -> list[tuple[str, str, float]]:
    ...
    for ligne in LEDGER.read_text(encoding="utf-8").splitlines():
        ...
        statut = cols[4].lower() if len(cols) >= 6 else ""
        if "retenu" not in statut:
            continue
        # Un modèle purgé porte souvent le mot « retenu » dans la phrase qui explique la
        # purge (« faisait passer le cumul retenu à 22,11 Go ») : la sous-chaîne seule
        # ferait échouer `doctor` sur un poids volontairement supprimé — cas du Chromium de
        # Puppeteer, purgé le 16/09/2026 et réclamé par le contrôle jusqu'à l'étape 13.2.
        if "purg" in statut:
            continue
