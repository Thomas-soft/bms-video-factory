...
def cible_de_rythme(
    ...
    for cle in ("cible_montage", "cible", "fallback_provisoire_s"):
        ...
        if isinstance(valeur, (int, float)):
            return float(valeur), facteur_hook, f"registre/REFERENTIEL.json → {niche_nom}.{cle}"
...
def construire_fenetres(script: Script, timings: Timings, words: Words) -> list[Fenetre]:
    ...
    for index, unite in enumerate(timings.segments):
        ...
        ids = [unite.id, *unite.merged_from]
...
def _asset_request(
    ...
    if type_asset == "card":
        ...
        mots_cles = plan.segment.visual_intent
    else:
        mots_cles = _intention(plan.segment, 0 if plan.premier_du_segment else 1, plan.rupture)
    return (
        AssetRequest(
...
def mettre_au_contrat(
    ...
    for rang, plan in enumerate(plans):
        ...
        demande, template = _asset_request(plan, rang, moteur, texte, template_precedent)
