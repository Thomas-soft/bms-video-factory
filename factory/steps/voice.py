...
def _synthetiser(unites: list[Unite], chemins: RunPaths, voix: str, lang: str,
                 racine: Path, journal: Path, force: bool,
                 locuteur: str | None = None) -> tuple[float, list[dict]]:
    ...
    **Pourquoi une relance et pas une simple erreur.** Mesuré le 16/09/2026 : sur un segment de
    ...
    travail = {
        ...
        "speaker": locuteur or tts.locuteur(voix),
        "force": force,
        "units": [
...
def _nettoyer(unites: list[Unite], chemins: RunPaths, langue: object,
              alertes: list[str], profil: object = None) -> list[Path]:
    ...
    """
    fichiers: list[Path] = []
    # Le profil de voix (config/voices, étape 29) module la vitesse de la langue.
    vitesse = langue.tts.speed * (profil.speed if profil else 1.0)  # type: ignore[attr-defined]
    demi_tons = profil.pitch_semitones if profil else 0.0  # type: ignore[attr-defined]
    ...
    for index, unite in enumerate(unites):
        ...
        if abs(demi_tons) >= 0.05:
            decale = chemins.voice_dir / f".pit_{index:02d}.wav"
            audio.hauteur(source, decale, demi_tons)
            source = decale
...
def executer(video_id: str, racine: Path | None = None, force: bool = False) -> ResultatVoix:
    ...
    profil = cfg.profil_voix(channel)
    if voix is None:
    ...
    if not force and all(fichier.exists() for fichier in deja_la):
        ...
        secondes_tts, comptes = _synthetiser(
            ...
            locuteur=profil.speaker if profil else None,
        )
        fichiers = _nettoyer(unites, chemins, langue, alertes, profil)
