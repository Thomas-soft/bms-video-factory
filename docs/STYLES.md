...
| **Whiteboard animé** | FLUX (trait noir sur blanc) + vtracer + `stroke-dashoffset` + Playwright + Revideo | **image 4,7/5** *(session)* · **animation 4/5** *(Thomas, 15/09, sur un clip de 5 s)* | **29,0** (12 × [137,0 s d'image + 7,9 s de capture]) | 4,64 Go (FLUX + vtracer 0,02) + Playwright 0,21 | `benchmarks/samples/visuel/img_whiteboard_1280x720.png` · `benchmarks/samples/visuel/whiteboard.svg` (27 chemins) · `benchmarks/samples/visuel/clip_whiteboard_720p.mp4` (1280×720, 150 img, 5,00 s, vérifié `ffprobe`) | débit seulement | **retenu v2** — étape 30.2 |
...
| **Avatar parlant** (2D) | personnage en calques (Open Peeps / Humaaans CC0) + Rhubarb 1.14 `-r phonetic` sous Rosetta 2 + Revideo | **visèmes : non évalué** (la session ne perçoit pas le son) · **avatar animé : jamais produit** | **0,12** (Rhubarb : 6,2 s pour 53 s de français, 309 visèmes) **+ le coût du fond** | 0,17 Go (Rhubarb) | `benchmarks/samples/visuel/visemes_phonetic.json` (309 visèmes) · `benchmarks/samples/visuel/lipsync_src.wav`. **Aucune preuve visuelle** | **rien** sur le lip-sync (6,2 s en CPU). La voix ne dépend pas du GPU : c'est un problème de données (RESULTATS § 2.9) | **retenu v2** — étape 29, **sous réserve de mesure** (§ 7) |
...
*Mise en forme WhatsApp (`*gras*`, ni titres ni tableaux), **19 lignes non vides**,
580 mots. **Thomas décide de l'envoi.** Texte brut, identique à ce bloc et vérifié par
...
>
> 1. Illustration animée (images générées + effet de profondeur)
> 2. Motion design
...
>
> Deux sont déjà validés sur des rendus que j'ai vus (illustration animée, whiteboard) ; les 4 autres, je les valide sur un rendu complet avant de les mettre en service. L'avatar 2D se pose par-dessus les autres styles, donc ça fait plus de rendus différents que 6 — et la vraie variété vient de la charte de chaque chaîne (palette, typo, rythme de coupe) : deux chaînes dans le même style ne se ressemblent pas.
>
...
>
> *Volume* : tout tourne sur mon MacBook Air M2 16 Go, qui est aussi ma machine de travail. Entre 3 et 7 vidéos par semaine environ selon le format. 10, non.
>
...
>
> *La suite* : prochain objectif, une vidéo de démo complète, de l'idée à la miniature, produite par le système. Puis une démo par style. En parallèle j'ouvre le dossier de publication, c'est le vrai chemin critique : YouTube force en privé tout ce qui sort d'un projet développeur non audité, et l'audit prend des semaines à des mois.
>
> *Il me faut 2 choses de ta part* :
> — raison sociale, SIREN et adresse de BMS
> — les comptes Google sous lesquels les chaînes doivent vivre (ça débloque aussi la banque de musique de YouTube, sans laquelle nos vidéos sortiraient sans musique de fond)
>
> Thomas
