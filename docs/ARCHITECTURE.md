...
- **Implémentation (étape 24, 23/09/2026).** `factory/steps/localize.py`. `creer_enfant` écrit `spec.json` (`parent_id`, langue, niche et durée de la cible, graine tirée jusqu'à ce que `template_id` **et** gabarit de miniature diffèrent du parent), copie `research.json`, écrit `heritage.json` (images du parent par demande d'asset, puis par segment) et marque `plan`/`research` faits. Le DAG reprend à `script`, qui appelle `adapter_script` dès que `spec.parent_id` est posé : adaptation par lots de 5 segments avec consignes et glossaire de `config/languages/<lang>.yaml` (`localisation`), ids, rôles, boucles, ruptures et `visual_intent` conservés, hook régénéré (même type, trois candidats notés), dérive de langue détectée, `retention/verify` avec deux reprises. `GenerateurImages.image_heritee` sert l'image du parent avant toute recherche en bibliothèque. Mesures au manifeste enfant (`declinaison`), ligne `children[]` au parent, `localizations` du parent remplies depuis les métadonnées de l'enfant. L'orchestrateur enfile les enfants des chaînes `derive_from` après `export` ou après upload (`orchestrator.yaml`, `declinaison.declencheur`) ; le calendrier refuse une date à moins de 24 h du parent.
...
**Les noms de tables ne sont pas libres.** `ROADMAP.md` les fixe déjà, migration par migration : `jobs`, `events`, `review_log` (22.1, 22.2) · `channels_watch`, `videos_ext`, `video_snapshots`, `channel_snapshots`, `collect_runs` (18) · `topics_queue`, `niche_scores` (19, 20) · `publications`, `quota_ledger`, `reporting_jobs` (23.1) · `perf_daily`, `perf_traffic`, `perf_reach`, `retention_curves`, `analytics_runs`, vue `v_video_perf` (25). Ce document et `INTERFACES.md` emploient **ces noms-là** et aucun autre : un schéma inventé ici obligerait vingt sessions à choisir entre deux vérités.

`events` est l'**index** du journal, pas le journal : `workspace/logs/events.jsonl` fait foi (§ 7) et la table se reconstruit en le relisant. Elle existe pour qu'une question du genre « toutes les erreurs du job 12 » soit une requête et non un `grep`.
