...
### 9.1 Politique de conservation de l'entrepôt concurrentiel (étape 18, 20/09/2026)

**Le texte, relu à la source le 20/09/2026** (https://developers.google.com/youtube/terms/developer-policies) :

- **III.E.4.d** — « [API Clients] may temporarily store limited amounts of [Non-Authorized Data] for as long as is necessary for the purposes of the [API Client] but **not longer than 30 calendar days**. »
- **III.E.4.b** — l'exception qui autorise à garder les statistiques (« the number of views for a video… ») **sans limite de durée** ne porte que sur les **Authorized Data**, et impose de vérifier tous les 30 jours que l'autorisation tient.
- **IV, définitions** — *Authorized Data* : données qu'un utilisateur autorise expressément via ses *User Credentials*. *Non-Authorized Data* : données accessibles **sans** *User Credentials*.
...
**Ce que cela impose ici.** L'entrepôt interroge des **chaînes tierces avec une clé API**, jamais avec les identifiants d'un utilisateur : tout ce qu'il stocke est donc **Non-Authorized Data**. L'exception III.E.4.b **ne s'applique pas**, y compris aux compteurs de vues. La limite est **30 jours pour tout**, sans distinction entre métadonnées et statistiques. (Elle s'appliquera en revanche à l'étape 25, sur nos **propres** chaînes en OAuth.)

**Politique appliquée**

| Donnée | Table | Conservation | Mécanisme |
|---|---|---|---|
| Métadonnées de vidéos tierces (titre, début de description, durée, tags, catégorie, miniature) | `videos_ext` | 30 jours **glissants** : rafraîchies à chaque collecte (`last_seen_at`) ; supprimées si elles cessent de l'être | `factory editorial purge` |
| Instantanés de vues, likes, commentaires | `video_snapshots` | 30 jours fermes à partir de `snapshot_date` | `factory editorial purge` |
| Instantanés d'abonnés, vues, nombre de vidéos | `channel_snapshots` | 30 jours fermes | `factory editorial purge` |
| Liste des chaînes suivies (identifiant, niche, langue) | `channels_watch` | conservée : c'est **notre** liste de veille, saisie par nous, pas une donnée rendue par l'API | — |
| Mesures dérivées (vélocité, vues à 1/7/30 jours, ratio) | `video_metrics` | **contestée — voir ci-dessous** | `purge --strict` les supprime |

**Rafraîchissement.** Une vidéo dont `last_seen_at` cesse d'avancer a disparu de l'API — supprimée, passée en privé, ou chaîne désactivée. Elle est purgée avec ses instantanés au bout de 30 jours. C'est ce qui satisfait l'obligation de ne pas conserver de données sur un contenu supprimé, sans avoir à interroger l'API à part.

**Suppression sur demande.** `factory editorial watch remove <chaîne>` la désactive immédiatement ; ses données cessent d'être rafraîchies et disparaissent sous 30 jours. Pour une suppression immédiate : `DELETE FROM videos_ext WHERE channel_id = …` puis `factory editorial purge`.

**Point ouvert — III.E.4.h contre `ARCHITECTURE.md` § 9.** Ce document (§ 9, phrase « Conserve indéfiniment, en revanche, les mesures dérivées ») et `ARCHITECTURE.md` § 9 posent depuis l'étape 1 qu'une mesure dérivée « n'est plus une donnée API » et se garde sans limite. **Le texte III.E.4.h dit l'inverse** : il interdit d'utiliser des API Data pour créer des métriques dérivées. La vélocité est précisément cela. Deux lectures :

1. *stricte* — la vélocité elle-même est hors politique, et l'étape 18 ne peut garder qu'une fenêtre glissante de 30 jours. C'est `purge --strict`, et cela prive la thèse n° 1 de son historique long ;
2. *usage interne* — III.E.4.h vise ce qu'un client **affiche ou substitue** aux données de YouTube ; un indicateur calculé pour notre seule décision éditoriale, jamais publié ni affiché à un tiers, n'entre pas dans cette interdiction.

Le système applique la lecture 2 **par défaut**, comme le voulait la doctrine déjà écrite, et ne publie ni n'expose aucune de ces mesures. `factory editorial purge --strict` applique la lecture 1 en une commande. **Arbitrage : Thomas, avant l'étape 25** (`SUIVI.md` § 1). Il n'a pas été tranché dans la session de l'étape 18 : le constat est neuf, et il change une décision d'architecture.

**Conséquence à assumer sur la valeur de l'entrepôt.** L'étape 18 promettait une donnée « qui s'apprécie avec le temps ». Avec la limite de 30 jours sur les données brutes, ce qui s'accumule vraiment est la **série de mesures dérivées** ; les compteurs bruts, eux, ne sont jamais qu'une fenêtre de 30 jours. Aucune ligne de code n'y change quoi que ce soit : c'est le contrat de l'API.
...
## 12. Opérationnel — comptes, publication, bascule post-audit (étape 23.2)

Ajoutée le 23/09/2026. Les sections 1 à 11 disent **quoi** ; celle-ci dit **qui fait quel geste, dans quel ordre**. Commandes exactes, aucune à lancer « pour voir » sur une chaîne réelle.

### 12.1 Checklist par compte — avant la première vidéo

À cocher **pour chaque chaîne**, puis à reporter dans `config/channels/<id>.yaml` (`google_account.*`). `factory precheck` refuse toute vidéo d'une chaîne où l'un des champs est faux (contrôles 24-25).

| # | Geste | Où | Champ de config |
|---|---|---|---|
| 1 | Compte Google **dédié** à la chaîne, créé par BMS (jamais le compte personnel d'Alek, Sofiane ou Thomas) | accounts.google.com | `google_account.alias` |
| 2 | Chaîne créée comme **Brand Account** détenu par BMS ; propriétaire principal = compte BMS, gestionnaires nommés | YouTube → Paramètres → Ajouter ou gérer des gestionnaires | `brand_account`, `owner: BMS` |
| 3 | **2FA** activée (clé physique ou application ; pas le SMS seul) | myaccount.google.com → Sécurité | `two_fa_enabled: true` |
| 4 | **Vérification téléphonique** de la chaîne | youtube.com/verify | `phone_verified: true` |
| 5 | Adresse de récupération = boîte BMS, pas une boîte personnelle | Sécurité du compte | — |
| 6 | Jeton OAuth : `factory publish auth --channel <id>`, choisir **la chaîne** (Brand Account), pas le compte | poste d'exploitation | `token_ref` |
| 7 | `youtube.channel_id` et `playlist_id` reportés (`factory publish auth --channel <id> --show`) | config | `youtube.*` |
| 8 | Jobs de rapports créés **avant** le premier upload : `factory publish reporting-jobs --channel <id>` | poste d'exploitation | — |

**Ce que débloque la vérification téléphonique** (fonctionnalités « intermédiaires », support.google.com/youtube/answer/171664) : miniatures personnalisées, vidéos de plus de 15 minutes, diffusions en direct, recours contre une revendication Content ID. Sans elle, `thumbnails.set` échoue (50 unités perdues, geste Studio) et un format long > 15 min est refusé.

**Un numéro, un nombre limité de chaînes.** Google plafonne le nombre de chaînes qu'un même numéro peut vérifier **sur une période donnée** (la page d'aide indiquait 2 chaînes par numéro et par an ; **à relire sur la page au moment du geste**, la valeur a déjà changé). Conséquences : prévoir **un numéro par tranche de deux chaînes** et noter dans le registre de comptes (hors dépôt) quel numéro a vérifié quelle chaîne et à quelle date ; **pas de numéro virtuel ni VoIP** (refusés ou révoqués) ; un numéro réutilisé sur beaucoup de chaînes est lui-même un lien entre elles.

**Appareil et réseau.** Les connexions à Studio se font depuis le **poste d'exploitation** de BMS, pas depuis un téléphone personnel ni un navigateur partagé avec le compte de recherche (§ 9, décision 2). Aucun VPN ni proxy commercial : une IP de datacenter partagée par plusieurs chaînes est un signal d'exploitation coordonnée. Le compte de recherche ne se connecte jamais sur ce poste.

### 12.2 Publication manuelle dans Studio — tant que l'audit n'est pas obtenu

Le daemon téléverse chaque vidéo **en privé**, entre 12 et 60 h avant sa date (avance tirée par vidéo, `calendrier.avance_upload_h`), **une seule par tour** et à 2 h au moins de la précédente, après un `precheck` PASS. Une date déjà passée n'est jamais téléversée : elle devient une violation de `calendar show --check`. La date reste une intention ; le geste public est humain.

1. Chaque matin, le digest de 7 h 30 liste les vidéos à programmer. Détail : `factory publish manual-list` — titre, chaîne, **date et heure dans le fuseau de l'audience**, lien Studio direct. Le pense-bête du run est `workspace/runs/<id>/publication.md`.
2. Ouvrir le lien Studio (connecté **au compte de cette chaîne seulement**).
3. Vérifier titre, description (mention IA en tête ; mention publicitaire en première ligne si produit), miniature, sous-titres, « Contenu altéré ou synthétique » conforme à `precheck.json`.
4. Si la chaîne porte des produits : cocher **« Inclut une communication commerciale / promotion payante »** (aucune API ne le fait, § 3 couche 2).
5. Visibilité → **Programmer** → date et heure **exactes** de `manual-list` (elles portent déjà le jitter ; ne pas arrondir à l'heure pleine).
6. Aucune autre action : pas de partage croisé entre chaînes BMS, pas de commentaire épinglé depuis un autre compte.

Compter ≈ 2 min par vidéo. Une vidéo non programmée à sa date **n'est pas rattrapée en rafale** : `factory calendar plan --replan` lui trouve un nouveau créneau.

### 12.3 Bascule post-audit

Quand Google accorde l'audit (courriel au propriétaire du projet `bms-factory`) :

1. Pour **chaque** chaîne auditée : `youtube.audit_passed: true` dans `config/channels/<id>.yaml`, puis `factory config validate`.
2. `factory calendar show --check` : 0 violation, sinon `factory calendar plan --replan`.
3. `factory publish release --en-attente` : chaque vidéo privée reçoit `publishAt` = sa date de calendrier. Les dates déjà passées sont listées et **ignorées** (pas de publication immédiate en série).
4. `factory publish manual-list` doit rendre une liste vide pour ces chaînes.
5. Le geste Studio **« promotion payante » reste manuel pour toujours** sur les chaînes affiliées : la bascule supprime le clic de publication, pas celui de divulgation.

### 12.4 Ajouter une chaîne

1. Comptes : les 8 lignes du § 12.1.
2. Config : `config/channels/<id>.yaml` — langue, niche, style, **3 templates au moins**, **voix différente** de toute chaîne de même langue, charte (palette, polices) distincte ; `cadence` : `per_week_max ≤ 2`, `days` = les 3 jours du `REFERENTIEL` (`niches.<niche>.creneaux.jours`, traduits), `hours_local` = heure dominante de la niche **convertie** de l'UTC vers le fuseau de l'audience, et **décalée d'au moins 2 h** des chaînes existantes publiant les mêmes jours, `jitter_min: 90` ou plus, `created_at` = date de création.
3. `factory config validate` (alerte si même langue et même niche qu'une autre chaîne : § 5).
4. Jeton et rapports : `factory publish auth --channel <id>`, puis `factory publish reporting-jobs --channel <id>`.
5. File : `factory queue add --channel <id> --n <k>`, puis `factory calendar plan --weeks 3` et `factory calendar show --check`.

### 12.5 Ce qu'on ne fait jamais

- **Outils d'engagement** : achat ou échange de vues, likes, abonnés, commentaires ; « sub4sub » ; commentaires croisés entre chaînes BMS ; aucun appel automatisé à `commentThreads.insert`, `subscriptions.insert` ou `videos.rate` (§ 5, § 9).
- **Automatisation de navigateur** sur youtube.com ou studio.youtube.com (Playwright, Selenium, extensions) : le geste Studio reste humain (§ 2).
- **Republication de contenu tiers** : aucun extrait de vidéo tierce, aucun yt-dlp, aucune « compilation » (§ 8, § 9).
- **Rafales** : pas de rattrapage de retard en publiant plusieurs vidéos le même jour ; jamais deux chaînes BMS dans la même heure, jamais deux chaînes de même langue à moins de 4 h ; jamais plus de 2 vidéos BMS sur 24 h glissantes ; jamais un horaire fixe (§ 6, `orchestrator.yaml § calendrier`).
- **Contournement du precheck** : pas de `publish upload` sur un run FAIL. `--force` ne lève que le contrôle des jobs de rapports, jamais la checklist.
- **Relever la cadence** avant 90 jours, ou de plus de 60 % d'un trimestre à l'autre (`TRAJECTOIRES.md` T4), sans décision écrite du client.

### 12.6 Seuils de la checklist (contrôles 18-19, durée)

Dans `config/qc.yaml § precheck`, comparés à **toute vidéo publiée de même langue, toutes chaînes BMS confondues** :

| Seuil | Valeur | Mesure | Lecture |
|---|---|---|---|
| `script_simhash_distance_min` | 12 bits sur 64 | SimHash des trigrammes de mots de la narration | en dessous = même script, **bloquant** |
| `thumbnail_phash_distance_min` | 10 bits sur 64 | pHash (imagehash) de `thumbnail.png`, comparé **toutes langues** | en dessous = même miniature, **bloquant** |
| `duree_min_s` | 60 s | `ffprobe` sur `final.mp4` | en dessous = **bloquant** |

Les deux distances sont des **ordres de grandeur à recalibrer** sur les premières vidéos publiées : aucune paire réelle de vidéos BMS n'a encore été mesurée (0 publication au 23/09/2026).

### 12.7 Objections — contradicteur « sécurité des comptes » (23/09/2026)

| # | Problème relevé | Suite |
|---|---|---|
| 1 | Le daemon enverrait toutes les vidéos dues d'un coup, avance fixe de 36 h | **Corrigé** : un upload par tour, 2 h mini entre deux, avance tirée 12-60 h. **Reste** : un seul projet GCP, un seul poste (décision 1 de ce document, assumée) |
| 2 | Rattrapage en rafale des dates passées | **Corrigé** : dates passées exclues de l'upload ; violation `date_depassee` ; precheck bloque |
| 3 | Templates et polices partagés entre langues ; pHash comparé en même langue seulement | **Partiel** : pHash désormais toutes langues. Templates par chaîne **non corrigés** — les chaînes FR/ES sont dormantes (anglais seul) ; à trancher avant toute réactivation (étape 24) |
| 4 | Même locuteur « serena » en EN, FR, ES | **Non corrigé** (chaînes FR/ES dormantes) ; règle : un locuteur par chaîne, portefeuille entier, avant l'étape 24 |
| 5 | `bms-test` clone de science-en | **Consigne** : ne jamais la créer sur YouTube ; aucune file ni date |
| 6 | Une heure par chaîne ; science-en et histoire-en proches | **Corrigé** : 2 heures par chaîne (REFERENTIEL), 3 jours, écart de 4 h entre chaînes de même langue |
| 7 | Plafond de 2/jour contournable à minuit | **Corrigé** : 24 h glissantes |
| 8 | Studio n'accepterait que des pas de 15 min, ce qui effacerait le jitter | **Non vérifié** — à constater au premier geste Studio (Thomas) ; ajout d'un avertissement de dispersion des heures |
| 9 | Descriptions au même squelette, nom du modèle en crédit, scories (« in a moment. ») | **Non corrigé ici** (générateur de métadonnées, étape 21) ; la relecture humaine doit les voir |
| 10 | Durées alternées ×0,85/×1,15, prévisibles | **Corrigé** : facteur tiré dans [0,8 ; 1,25] |
| 11 | Toujours l'un des 2 premiers créneaux | **Corrigé** : tirage parmi 3 ; dates dans l'ordre de production |
