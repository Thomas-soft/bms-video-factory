...
> **Verdict de Thomas sur les deuxièmes versions, 16/09/2026 :** *« jusqu'à présent y'a que le
> B qui est intéressant »*, et pour C : *« c'est pas cohérent le visuel, des images qui se
> découpent… vraiment pas très cohérent visuellement »*, et pour A : *« trop trop de texte pour
> presque aucune image, aucune animation, que du texte quasiment »*. **A et C échouent une
> seconde fois, après réécriture complète : ce n'est pas un défaut d'exécution, c'est le
> traitement** — et pour A, c'est sa prémisse même, donc une v3 serait encore du texte.
>
> **Ce qui unifie les trois verdicts est la vraie sortie de cette preuve.** A anime en
> permanence (chaque mot bouge, la caméra bouge, cinq sortes de transition) et se lit « aucune
> animation » ; C déplace des fragments et se lit « pas cohérent » ; B construit et se lit
> intéressant. **Ce qui est perçu comme animation est la construction et la transformation,
> jamais le déplacement.** C'est mot pour mot le reproche fait aux 127 plans de l'étape 12.2,
> appliqué cette fois à du texte et à des fragments d'image. Ce qui suit reste vrai comme
> mesure ; ce qui change est la conclusion, portée au § 6.
...
32 s. **Rien de cela ne se généralise tout seul à un autre sujet.** A (421 lignes) et
C (374 lignes) se rejouent sur n'importe quel script sans être réécrits ; B non.
...
1. ~~Regarder les trois MP4 en lecture et noter chacun.~~ **✅ fait le 16/09/2026 :
   seul B est retenu.** A et C sont écartés après deux exécutions chacun. **Décider si
   un troisième essai leur est accordé — par défaut, non**, et si l'étape 30.1 part
   donc sur le schéma animé seul.
...
4. **Trancher le prochain essai.** Le seul traitement retenu est celui qui ne se
   généralise pas. La mesure qui manque n'est pas une quatrième esthétique, c'est
   **ce qui reste de B sur un autre sujet sans réécrire ses 43 repères**. Trois
   routes, aucune mesurée : gabarits paramétrés choisis par `visual_intent` ;
   chorégraphie écrite par le LLM local ; vectorisation `vtracer` d'une image FLUX
   puis animation des chemins.
