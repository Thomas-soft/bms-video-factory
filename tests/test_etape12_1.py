...
def test_une_unite_de_voix_fusionnee_ne_dedouble_pas_son_segment() -> None:
    """`merged_from` porte DÉJÀ l'id de l'unité : sans déduplication le segment prend deux
    fenêtres et les bornes se répartissent sur un identifiant fantôme.

    Mesuré le 18/09/2026 sur le run EN `avwf`, où la voix a fusionné trois segments :
    31 fenêtres pour 28 segments, 141 plans qui se recouvrent, un plan de 273,8 s et un
    `assembled.mp4` de 1852 s pour une voix de 725 s. Le défaut dormait depuis l'origine —
    aucun run antérieur n'avait de segment assez court pour être fusionné.
    """
    script = script_de([segment("seg_00", "hook"), segment("seg_01"), segment("seg_02")])
    timings = m.Timings(
        voice_id="v", engine="qwen3_tts", loudness_lufs=-14.0, true_peak_dbtp=-1.4,
        total_duration_s=40.0, segments=[
            m.SegmentVoix(id="seg_00", file="a.wav", start_s=0.0, end_s=12.0, duration_s=12.0,
                          text="a"),
            m.SegmentVoix(id="seg_01", merged_from=["seg_01", "seg_02"], file="b.wav",
                          start_s=12.5, end_s=39.5, duration_s=27.0, text="b"),
        ])
    words = m.Words(engine="whisper", fallback_reason="coverage_below_threshold", coverage=1.0,
                    wer_vs_script=0.02, wer_threshold=0.08,
                    words=mots(0, 12, "seg_00") + mots(12.5, 25, "seg_01")
                          + mots(25.5, 39.5, "seg_02"))
    fenetres = sl.construire_fenetres(script, timings, words)

    assert [f.segment.id for f in fenetres] == ["seg_00", "seg_01", "seg_02"]
    assert fenetres[0].debut_s == 0.0
    assert fenetres[-1].fin_s == pytest.approx(40.0)
    for gauche, droite in zip(fenetres, fenetres[1:]):
        assert gauche.fin_s == pytest.approx(droite.debut_s)
    # la somme des fenêtres ne peut pas dépasser la piste : c'est ce que le doublon cassait
    assert sum(f.duree_s for f in fenetres) == pytest.approx(40.0)
...
def test_un_style_planifie_nomme_l_etape_qui_le_livrera() -> None:
    # Le témoin tourne à chaque livraison : `illustre_anime` (12.2), `documentaire` (17),
    # `avatar2d` (29). Depuis l'étape 29, tous les moteurs de `config/styles/` sont livrés :
    # le témoin est un moteur fictif, et le message renvoie à ROADMAP.md.
    planifie = m.Style(id="avatar_realiste", engine="avatar_realiste", backend="ffmpeg",
                       statut="planifie")
    with pytest.raises(MoteurIndisponible, match="ROADMAP"):
        get_engine(planifie)
...
def test_la_precision_de_cadrage_atteint_le_prompt_et_pas_seulement_l_intention() -> None:
    """Six plans d'un même segment doivent demander six images, pas deux.

    Régression du 18/09/2026, mesurée sur `bms-science-fr-20260917-rtmk` : `visual_intent`
    portait bien les quatre `PRECISIONS`, mais `_asset_request` figeait le rang à 0/1 — tout plan
    non initial demandait « — plan large ». Résultat à l'écran : **30 prompts distincts pour
    124 plans**, un seul servant **22 plans**, et 36 images pour 12 minutes de vidéo.
    """
    plans = [sl.PlanBrut(segment("seg_01"), float(i) * 5, float(i) * 5 + 5, i == 0, False)
             for i in range(6)]
    shots = sl.mettre_au_contrat(plans, "vid", chaine(), "illustre_anime", set(),
                                 sl.ALTERNANCE_MOUVEMENT)
    prompts = [s.asset_request.prompt_or_keywords for s in shots]
    # 1 intention nue + 4 précisions, puis le cycle reprend : 5 prompts distincts sur 6 plans.
    assert len(set(prompts)) == 5, prompts
    assert prompts[1].endswith("plan large")
    assert prompts[2].endswith("détail")
    assert prompts[3].endswith("variation de cadre")
    assert prompts[4].endswith("contre-champ")
    # Le prompt suit l'intention mot pour mot : c'est ce lien qui était rompu.
    assert prompts == [s.visual_intent for s in shots]


def test_un_plan_de_rupture_ne_retombe_pas_sur_le_prompt_de_son_segment() -> None:
    """La rupture passe par la branche `stock`, qui servait l'intention nue du segment.

    Sur le run du 17/09, les 27 plans de rupture demandaient donc **exactement** le prompt du
    premier plan de leur segment, et la bibliothèque leur resservait la même image.
    """
    rupture = m.Interrupt(type="chiffre", at_s_relative=5.0)
    plans = [sl.PlanBrut(segment("seg_01", rupture=rupture), 0.0, 5.0, True, False),
             sl.PlanBrut(segment("seg_01", rupture=rupture), 5.0, 10.0, False, True)]
    shots = sl.mettre_au_contrat(plans, "vid", chaine(), "illustre_anime", set(),
                                 sl.ALTERNANCE_MOUVEMENT)
    assert shots[1].asset_request.type == "stock"
    assert (shots[1].asset_request.prompt_or_keywords
            != shots[0].asset_request.prompt_or_keywords)
    assert shots[1].asset_request.prompt_or_keywords.endswith("rupture visuelle, cadre et fond changés")


def test_une_intention_qui_nomme_un_corps_declenche_le_controle_27() -> None:
    """`CONFORMITE` § 3 couche 3 est **bloquant** — il ne peut pas être du code mort.

    Régression du 18/09/2026 : `AssetRequest.contains_person` était la constante `False`, donc
    `virtual_images_mention` ne pouvait jamais valoir `True` sur aucun run. Mesuré sur
    `bms-science-fr-20260917-rtmk` : 0 plan sur 124 déclaré, alors que 20 nomment un corps.
    """
    assert sl.figure_une_personne("Des mains saisissent une barre de fer.") is True
    assert sl.figure_une_personne("A runner crossing the finish line") is True
    assert sl.figure_une_personne("Silueta de una persona caminando") is True
    # Mot à mot, pas sous-chaîne : « domaine » contient « main » et ne doit pas déclencher.
    assert sl.figure_une_personne("Un domaine de recherche vaste") is False
    assert sl.figure_une_personne("Diagram of light rays") is False


def test_le_drapeau_personne_descend_jusqu_a_la_demande_d_asset() -> None:
    corps = segment("seg_01")
    corps.visual_intent = "Des mains saisissent une barre de fer"
    neutre = segment("seg_02")
    neutre.visual_intent = "Un diagramme de rayons lumineux"
    plans = [sl.PlanBrut(corps, 0.0, 5.0, True, False),
             sl.PlanBrut(neutre, 5.0, 10.0, True, False)]
    shots = sl.mettre_au_contrat(plans, "vid", chaine(), "illustre_anime", set(),
                                 sl.ALTERNANCE_MOUVEMENT)
    assert shots[0].asset_request.contains_person is True
    assert shots[1].asset_request.contains_person is False
