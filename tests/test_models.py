...
def exemples() -> dict[str, m.ModeleRacine]:
    ...
    return {
        ...
        "OrchestratorConfig": CFG.orchestrator,
        "VideoSpec": _spec(),
        "Script": _script(),
        "Words": m.Words(
        ...
        "VideoMetadata": m.VideoMetadata(
            title_chosen="Le sucre est un code que personne ne lit",
            title_variants=[m.VarianteTitre(text="Le sucre est un code que personne ne lit",
                                            pattern="verite_choc", length_char=40, score=0.71)],
            description="Publicité\n\nAccroche.\n\n0:00 Hook",
            description_blocks=m.BlocsDescription(
                hook="Accroche.",
                chapters=[m.Chapitre(start_s=0, title="Hook"),
                          m.Chapitre(start_s=42, title="Le premier point"),
                          m.Chapitre(start_s=95, title="La suite")],
            ),
            tags=["sucre", "science"], category_id="28",
            default_language="fr", default_audio_language="fr",
            chapters=[m.Chapitre(start_s=0, title="Hook"),
                      m.Chapitre(start_s=42, title="Le premier point"),
                      m.Chapitre(start_s=95, title="La suite")],
        ),
...
def test_produit_subid_impose_par_le_reseau() -> None:
    ...
    assert produit.subid("bms-science-en", "en", "bms-science-en-20260918-k7q2") == \
        "bms-science-en-20260918-k7q2"  # sub_id_format "{video_id}" (étape 27)
