...
def test_base_creee_avec_ses_tables_et_idempotente(tmp_path) -> None:
    ...
    try:
        ...
        assert db.version_schema(conn) == 13  # 013 : index de bibliothèque (étape 29)
        assert "revenue" in db.tables(conn)
        assert {"publications", "quota_ledger", "reporting_jobs"} <= db.tables(conn)
        assert {"embeddings", "topic_clusters", "topics_queue"} <= db.tables(conn)
        ...
        assert {"video_id", "parent_id", "manifest_json", "cut_rhythm_measured_s",
                "reuse_ratio", "score_qc", "qc_verdict", "qc_version"} <= colonnes
    ...
    try:
        assert db.version_schema(conn2) == 13
    finally:
        conn2.close()
