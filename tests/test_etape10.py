...
from factory.llm import _nettoyer_sortie, extraire_objet, schema_vers_gbnf
...
def test_extraire_objet_ignore_le_gabarit_du_prompt() -> None:
    """L'écho du prompt porte un exemple équilibré mais non parsable : c'est la génération
    qui doit sortir, pas lui (défaut mesuré le 18/09/2026 sur le juge de boucle)."""
    texte = (
        'Answer with this JSON object and nothing else:\n'
        '{"answers": true or false, "why": "<at most 12 words>"}\n'
        'Rules:\n- `true` only if the ... (truncated)\n'
        '{"answers": true, "why": "Second passage confirms both facts."}'
    )
    brut = extraire_objet(texte)
    assert brut is not None
...
def test_nettoyer_sortie_coupe_un_echo_tronque() -> None:
    """`llama-cli` clôt son écho tronqué par « ... (truncated) » ; le marqueur des 120
    derniers caractères du prompt est alors introuvable et l'écho restait collé."""
    prompt = "Un prompt assez long pour que llama-cli tronque son écho." * 20
    sortie = (
        "bannière\n" + prompt[:80] + " ... (truncated)\n"
        '{"answers": false, "why": "hors sujet"}\nExiting...'
    )
    assert _nettoyer_sortie(sortie, prompt) == '{"answers": false, "why": "hors sujet"}'
...
def test_ligne_de_divulgation_orale_n_est_jamais_un_hashtag() -> None:
    ...
    for lang in ("fr", "en"):
        ...
        if surcharge is not None:  # étape 27 : la mention orale du produit prime, en toutes lettres
            assert lignes.spoken_line == surcharge.spoken_line
            continue
