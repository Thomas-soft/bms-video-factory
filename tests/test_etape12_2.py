...
def test_plan_court_bascule_en_ken_burns(style_illustre) -> None:
    ...
    Le test **ne lit plus le défaut de la configuration** : Thomas a rejeté la parallaxe le
    16/09/2026 et `config/styles/illustre.yaml` est passé à `ken_burns`, ce qui rendait la
    seconde assertion fausse pour une raison qui n'a rien à voir avec la durée. La règle de
    durée se teste donc sur un style qui demande la parallaxe, et le défaut en vigueur se
    teste pour ce qu'il est : aucun plan, quelle que soit sa durée, ne part en parallaxe.
    """
    parallaxe = style_illustre.model_copy(deep=True)
    parallaxe.params["motion"] = {**parallaxe.params.get("motion", {}), "default": "parallax"}
    moteur = IllustreEngine(style=parallaxe)
    assert not moteur._veut_parallaxe(_plan(duree=1.5))
    assert moteur._veut_parallaxe(_plan(duree=6.0))

    en_vigueur = IllustreEngine(style=style_illustre)
    assert en_vigueur.mouvement_defaut == "ken_burns"
    assert not en_vigueur._veut_parallaxe(_plan(duree=6.0))
