...
- **Langue de production : anglais uniquement.** Tranché par Alek le 15/09/2026, précisé par Thomas le 17/09/2026 : aucune version FR n'est produite, ni en double, ni en repli. Les chaînes FR, ES et IT du registre restent des **sources** de veille et de sujets. L'étape 24 (déclinaison multilingue) est **différée** jusqu'à une décision contraire d'Alek. Les prompts des étapes 13.2 à 30.2 ont été réécrits en conséquence le 17/09/2026 ; ceux des étapes closes (1 à 13.1) restent tels qu'exécutés.
...
```
 ...
 BANC QC (rythme, hook, durée, audio, lisibilité) ─► PORTILLON ─► régénération ou blocage
   │
   ▼
 publication (API : privé → programmé ; calendrier crédible ; checklist conformité) ─► déclinaisons multilingues (différées — anglais uniquement)
   │
   ▼
...
| 24 | 4 | Déclinaison multilingue par adaptation — **différée** (anglais uniquement) | `factory/steps/localize.py` | 1 | 62 500 tk — 31,2 % | 23.2, 16 |
...
**Terminé quand.** `final.mp4` : `ffprobe` confirme H.264 1920×1080 30 fps + AAC, durée = durée de la voix ±0,5 s · loudness intégrée -14 ±1 LUFS, true peak ≤ -1 dBTP · nombre de plans détectés par PySceneDetect = n_shots ±10 % · **sous-titres vérifiés selon la charte : piste `mov_text` présente, codée et étiquetée de la langue de la chaîne si `charte.subtitles.burn_in` est faux ; lisibles sur une image extraite s'il est vrai** · `music.licence` dans le manifeste · temps de montage et d'export dans le manifeste.

> **Amendement du 16/09/2026 (étape 13.1).** Le critère disait « sous-titres visibles sur une image extraite ». Il supposait l'incrustation, que **les quatre chaînes refusent** (`charte.subtitles.burn_in: false`, `youtube.captions_upload: true`) : la route est le mux, et l'objectif de l'étape dit lui-même « incrustés **ou** muxés ». Un sous-titre muxé n'est par construction sur aucune image extraite — le critère d'origine ne pouvait qu'échouer ou faire violer la charte. Il devient donc conditionnel à `burn_in`. Ce n'est pas un contournement : la piste muxée est vérifiée, et plus strictement qu'à l'œil (présence, codec, code de langue ISO 639-2). À noter pour l'étape 15 : la build ffmpeg de la machine est compilée **sans libass**, donc `burn_in: true` n'y est pas exécutable ; `factory assemble` échoue explicitement au lieu de retomber en silence sur le mux.
...
**Terminé quand.** `uv run factory run --channel bms-science-en`, exécuté deux fois sur deux sujets neufs, produit à chaque fois `final.mp4`, `thumbnail.png` 1280×720 avec texte lisible (hauteur de texte ≥ 12 % de l'image), `metadata.json` valide (titre ≤ 70 caractères, description avec chapitres, mention IA, bloc d'attribution, mention affiliation si produit) · `manifest.json` contient timings par étape, temps total, coût en euros, variantes de titres (≥ 5) et de textes de miniature (≥ 3) · aucune intervention manuelle pendant les deux runs · Thomas a visionné une vidéo et noté 3 défauts dans `STATE.md` (ou « non visionnée »).
...
5. Exécute `factory run --channel bms-science-en` sur un sujet neuf (le plan en choisit un non utilisé), journal workspace/logs/run_<id>.log (tail -40 seulement). Puis une seconde fois `factory run --channel bms-science-en` sur un autre sujet neuf. Anglais uniquement : aucun run FR. Aucune intervention manuelle : si une étape échoue, corrige le code et relance avec --from.
6. Regarde thumbnail.png du premier run et 2 images de qc/frames/ ; vérifie la lisibilité.
7. Demande à Thomas de visionner final.mp4 du premier run EN et de dicter 3 défauts (ou « non visionnée ») ; consigne-les dans STATE.md pour les étapes 15 et 16.
...
4. Le metadata.json du premier run EN de l'étape 13.2 (court).
</demarrage>
...
5. Exécute l'upload du premier run EN de l'étape 13.2 (anglais uniquement : jamais un run FR). Demande à Thomas de confirmer dans YouTube Studio que la vidéo apparaît en privé avec ses métadonnées.
...
<tache>
1. factory/retention/patterns_en.yaml (anglais uniquement ; le nommage `patterns_<lang>.yaml` reste prêt pour l'étape 24 différée) : patrons de hook par type (à partir du rapport A), formulations proscrites, marqueurs de boucle ouverte, phrases de paiement.
...
7. Exécute `factory run --channel bms-science-en --from script` sur un run existant ou un nouveau ; compare avant/après (mots, densité, hook, boucles) ; lance `factory qc`.
...
5. config/channels/bms-science-en.yaml (modèle). bms-histoire-en.yaml n'existe pas encore : il est créé dans cette session (voir tâche 5). bms-histoire-fr.yaml et bms-histoire-es.yaml ne sont plus des cibles de production.
</demarrage>
...
5. Crée config/channels/bms-histoire-en.yaml sur le modèle de bms-science-en.yaml (lang: en, niche: histoire_doc, voix EN retenue), puis exécute `factory run --channel bms-histoire-en --style documentaire` (sujet neuf), journal (tail -40). Lance `factory qc`. Compare cut_rhythm_measured_s à la cible pour ce run et pour le run illustré de la phase 1 (ou un nouveau run illustré si nécessaire) ; si l'écart dépasse 15 %, ajuste shotlist.py (jitter, placement des coupes) ou les seuils de détection du banc (transitions xfade détectées comme deux coupes ?) et documente le réglage dans docs/QC.md.
...
3. Vues SQL : v_video_velocity (vues à 1, 7, 30 jours par interpolation des instantanés, vélocité = Δvues/Δjours sur les 7 derniers jours, ratio vs médiane de la chaîne à âge égal) ; `factory editorial top --niche X --window 30d [--lang en]` affiche les 20 vidéos les plus « rapides » avec chaîne, âge, vues, vélocité, ratio.
...
**Objectif.** Noter chaque niche candidate, en anglais (la dimension langue reste dans le modèle de données, avec une seule valeur : en), sur la demande (signaux gratuits : Wikimedia, autocomplete YouTube, Google Trends si accès), la concurrence (entrepôt : chaînes actives, vélocité médiane, part de percées), le potentiel de monétisation (configuration : programmes d'affiliation disponibles par langue, bandes de CPM documentées) et la saisonnalité ; produire un classement avec preuves.
...
<tache>
1. config/editorial.yaml : pondérations (demande 35, concurrence 25, monétisation 25, faisabilité 15), seeds par niche, en anglais (5 à 10 articles Wikipedia et 5 mots-clés par niche, proposés par le LLM puis validés par règles : existence de l'article), grille de monétisation (par niche et langue : programmes d'affiliation disponibles selon docs/CONFORMITE.md et le rapport de l'étape 1, bande de CPM avec source et date, note 1-5), faisabilité (styles disponibles localement adaptés à la niche, sources factuelles gratuites disponibles).
...
<fin_de_session>
Mets à jour STATE.md : étape 19 terminée, top 3 niches (anglais), sources de demande réellement disponibles (Trends oui/non), limites. Commit. Résume en 10 lignes maximum.
</fin_de_session>
````
...
<contexte>
Projet : système autonome de production de vidéos YouTube faceless (BMS), 0 €, local. L'entrepôt (étape 18) enregistre chaque jour les vues des vidéos de ~70 chaînes suivies ; la notation des niches (étape 19) dit où produire. Cette session dit quoi produire : percées (vidéos qui font plusieurs fois la médiane de leur chaîne à âge égal), regroupement des sujets par embeddings multilingues locaux, trous (un sujet fort en français, en espagnol ou en italien et absent en anglais, espagnol ou italien — le modèle des réseaux Thoth ; une demande élevée avec peu de vidéos récentes), angles proposés par le LLM selon la persona de la chaîne, file de sujets notée. `factory plan` consommera cette file en priorité. Les sujets ne sont jamais copiés : ce sont des thèmes, l'angle et le script sont propres à BMS (docs/CONFORMITE.md §4-5).
</contexte>
...
5. reports/niches.md — tête du classement (anglais).
</demarrage>
...
2. factory/editorial/topics.py :
   ...
   - trous : (a) multilingues, dans un seul sens : clusters avec ≥ 2 percées en FR, ES ou IT et 0 ou 1 vidéo en anglais chez les chaînes EN suivies (l'anglais est la seule langue cible) ; (b) demande sans offre : clusters (ou seeds de config/editorial.yaml) à demande Wikimedia élevée et < 3 vidéos récentes chez les chaînes suivies ; (c) résurgences : clusters anciens qui regagnent de la vélocité ;
...
4. Exécute pour bms-science-en et bms-histoire-en ; puis `factory plan --channel bms-science-en` ; vérifie la source et l'absence de doublon inter-chaînes (crée un plan pour bms-histoire-en et vérifie qu'un sujet identique n'est pas repris).
...
<criteres_de_validation>
Montre : sqlite3 "select channel_id, count(*) from topics_queue where status='proposed' group by channel_id;" (≥ 20 chacun) ; les 5 premières lignes de reports/topics_bms-science-en.md ; la section trous multilingues ; le spec.json du nouveau plan (source, evidence) ; le résultat du test anti-doublon.
</criteres_de_validation>
...
4. tests/test_seo.py : chapitres valides, longueur de tags, sous-identifiant présent quand un produit est configuré et absent sinon, mentions présentes en anglais, titre ≤ 70 (≥ 8 tests).
5. Exécute `factory run --channel bms-science-en --from thumbnail` sur un run existant ; regarde les 3 miniatures ; vérifie metadata.json.
...
6. Tests : enfile 2 jobs pour bms-science-en (auto_approve temporairement true pour ce test, ou approuve avec une commande minimale) ; lance `factory daemon run-once` ; pendant le rendu du 2e, tue le processus (kill) ; relance ; vérifie dans events.jsonl la reprise à l'étape interrompue ; les deux jobs finissent exported. Lance backup run puis restore-test.
...
5. Charge la file pour le jalon : `factory queue add` sur ≥ 2 chaînes (bms-science-en et bms-histoire-en) pour ≥ 5 vidéos ; `factory calendar plan --weeks 3` ; `factory calendar show --check` (0 violation). Note dans STATE.md : le daemon va produire et publier pendant 3 semaines ; les actions humaines attendues (relecture, clic Studio si pas d'audit) ; la date de vérification du jalon.
...
> **DIFFÉRÉE — anglais uniquement** (Alek, 15/09/2026 ; précision de Thomas, 17/09/2026). **Exécutée partiellement le 23/09/2026 puis désactivée** (Thomas : « Pas de FR, full anglais ») : code livré et inactif, voir `SUIVI.md` § 3. Cette étape n'est lancée que sur décision explicite d'Alek d'ouvrir une seconde langue. Aucune étape suivante ne la liste en dépendance (§ 5). Le prompt est conservé tel quel, sans réécriture.
...
4. Exécute `factory learn` sur les données réelles ; puis `factory plan --channel bms-science-en --dry-run --n 10` avec et sans learned/weights.json (renomme temporairement) et montre le diff des deux listes.
...
5. config/channels/bms-science-en.yaml (charte actuelle).
</demarrage>
...
6. Exécute un run avec avatar2d en overlay (bms-science-en) : qc ; extrais 3 images à des instants de visèmes différents (lis la timeline) et regarde-les ; mesure le temps ajouté par l'avatar.
...
5. config/chartes/bms-science-en.yaml (issue de l'étape 29 ; à créer dans cette session si elle n'existe pas).
</demarrage>
...
3. Mesure : rend un run complet (bms-science-en, style motion) ; temps par plan, images par seconde de rendu, temps par minute de vidéo, pic mémoire de Chromium ; inscris dans benchmarks/RESULTATS.md §3 « Moteurs » ; si le rendu dépasse 8× temps réel, teste 1080p24 ou le rendu à 720p suivi d'un upscale ffmpeg et documente le compromis retenu.
...
3. Exécute un run complet en style whiteboard (bms-science-en ou une niche home_hacks si configurée) ; qc ; extrais 3 images à 20 %, 50 %, 80 % d'un même plan et regarde-les (la main suit-elle le trait ? le résultat est-il lisible ?) ; mesure le temps par plan et inscris-le dans RESULTATS.md §3.
